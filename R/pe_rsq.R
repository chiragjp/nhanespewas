...
#' }
#' @importFrom DBI dbGetQuery
#' @importFrom dplyr tbl filter select inner_join group_by summarize collect rowwise mutate ungroup pull bind_rows arrange na.omit
...
get_individual_level_table <- function(summary_stats_con, nhanes_con, pvarname_to_query) {
  ...
  selected_data <- big_cc |> select(all_of(selected_vars)) |> na.omit()
