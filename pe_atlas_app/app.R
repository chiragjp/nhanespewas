...
## get data
pool <- dbPool(drv = RSQLite::SQLite(), dbname='pe_summary_stats_shiny.sqlite')
##
...
e_category_strs <- e_variables |> select(ecategory, esubcategory) |> group_by(ecategory, esubcategory) |> count() 
...
ui <- navbarPage("Phenome-Exposome Atlas",
    ...
    tabPanel("Browse By Phenotype", 
        ...
        mainPanel(
            ...
            tabsetPanel(type = "tabs",
                ...
                tabPanel("Effect Sizes in Context", plotOutput("by_phenotype_context_plot"),  p("Displaying effect sizes for FDR less than 5%")),
                tabPanel("Var. Explained", plotOutput("by_phenotype_r2_plot")),
                tabPanel("Adjusted Effect Size", plotOutput("by_phenotype_estimate_plot"))
            ),
    ...
    tabPanel("Browse By Exposure", 
             ...
             mainPanel(
               ...
               tabsetPanel(type = "tabs",
                           tabPanel("-log(Meta-P) vs. Effect Size", plotOutput("by_exposure_volcano_plot"),p("Points in the background depict summary stats for the entire category")),
                           tabPanel("Effect Sizes in Context", plotOutput("by_exposure_context_plot"), p("Displaying effect sizes for FDR less than 5%")),
                           tabPanel("Var. Explained", plotOutput("by_exposure_r2_plot")),
                           tabPanel("Adjusted Effect Size", plotOutput("by_exposure_estimate_plot"))
               ),
               h3("Summary Statistics"),
    ...
    tabPanel("Browse by Exposure Group", 
             ...
             mainPanel(
               ...
               tabsetPanel(type="tabs",
                           ...
                           tabPanel("Exposure Group in Context", plotOutput("by_exposure_group_context_plot")),
                           tabPanel("Var. Explained", plotOutput("by_exposure_group_r2_plot")),
                           tabPanel("Adjusted Estimates", plotOutput("by_exposure_group_estimate_plot"))
                ),
    ...
    tabPanel("Inter-Survey Estimates & Meta-Analysis", 
             sidebarPanel(
                selectInput("pvarname_inter", h3("Phenotype"), choices=setNames(p_variables$pvarname,p_variables$pvardesc), selected="BMXBMI"),
                selectInput("evarname_inter", h3("Exposure"), choices=setNames(e_variables$evarname,e_variables$evardesc), selected="LBXBCD")
              ),
             mainPanel(
                plotOutput("survey_pair_plot"),
                h2("Per Survey Estimates"),
                reactableOutput("survey_pair_table"),
                h2("Heterogeneity Statistics"),
                reactableOutput("heterogeneity_table")
             )
    )
...
## meta-analysis
table_for_pair <- function(evarname_str, pvarname_str) {
  expos_wide <- tbl(pool, "expos_wide")
  dat <- expos_wide |> filter(evarname == evarname_str, pvarname == pvarname_str) |> collect() |> 
    select(Begin.Year, e_variable_description, p_variable_description, term, estimate_adjusted, std.error_adjusted, p.value_adjusted,nobs_adjusted) 
  
  dat <- dat |> rename(Survey=Begin.Year, estimate=estimate_adjusted, std.error=std.error_adjusted, p.value=p.value_adjusted,N=nobs_adjusted) |> mutate(expo_name=term)
  
  meta_dat <- tbl(pool, "adjusted_meta_2") |> 
    filter(evarname == evarname_str, pvarname == pvarname_str) |> 
    mutate(Survey = 'overall') |> select(Survey, evardesc, pvardesc, term, expo_name, estimate, std.error, p.value,total_n) |> collect()
  meta_dat <- meta_dat |> rename(e_variable_description=evardesc, p_variable_description=pvardesc,N=total_n)
  
  dat <- dat |> rbind(meta_dat) 
  if(any(dat$term == 'expo')) {
    dat <- dat |> select(-term)
  }
  dat
}

plot_pair <- function(dat, estimate_limits=c(0.01,.35)) {
  exposure_name <- dat$e_variable_description[1]
  phenotype_name <- dat$p_variable_description[1]
  dat <- dat |> mutate(point_shape = ifelse(Survey == 'overall', 23, 21)) |> mutate(point_shape = as.integer(point_shape)) 
  dat <- dat |> mutate(point_size = ifelse(Survey == 'overall', 7, 2)) 
  p <- ggplot(dat, aes(Survey, estimate))
  ...
  if('term' %in% colnames(dat)) {
    p <- p + facet_grid(~expo_name)
  }
...
plot_volcano <- function(to_plot, bg=NULL) {
  ...
  p <- p + scale_x_continuous(limits=c(-.2, .2)) + scale_color_aaas()
  p <- p + theme_bw() + xlab("Meta-Estimate (per 1 SD of scale(E) on scale(P))") + ylab("-log10(meta-pvalue)")
  p
}
...
reactable_tibble <- function(meta_obj) {
  to_react <- meta_obj |>
    select(pvardesc, evardesc, estimate,mean_adjusted_base_r2_diff, p.value, sig_levels, total_n, i.squared, nobs) |> collect() |> 
    mutate(neglogpvalue=-log10(p.value)) |> select(-p.value) |> 
    rename(r.squared=mean_adjusted_base_r2_diff,n_surveys=nobs, total_sample_size=total_n, p_description=pvardesc, e_description=evardesc)
  to_react <- to_react |> select(p_description, e_description, estimate, neglogpvalue, sig_levels, r.squared, i.squared, total_sample_size, n_surveys)
}

reactable_summary_stats <- function(meta_obj) {
  to_react <- reactable_tibble(meta_obj)
  reactable(to_react |> arrange(-neglogpvalue), columns = list(estimate=colDef(format = colFormat(digits=3)),
                                                               neglogpvalue=colDef(format=colFormat(digits=1)),
                                                               i.squared=colDef(format = colFormat(digits=2)),
...
server <- function(input, output, session) {
    ...
    output$by_phenotype_r2_plot <- renderPlot({
        pvarn <- input$pvarname
        to_plot <- tbl(pool, "adjusted_meta_2") |> filter(pvarname==pvarn) |> select(mean_adjusted_base_r2_diff,mean_unadjusted_r2_diff, sig_levels) |> collect()
        to_plot <- to_plot |> rename(r2_adjusted_vs_base=mean_adjusted_base_r2_diff, r2_adjusted_vs_unadjusted=mean_unadjusted_r2_diff)
        plot_adjusted_r2_cdf(to_plot)
    })
    
    output$by_phenotype_estimate_plot <- renderPlot({
        pvarn <- input$pvarname
        to_plot <- tbl(pool, "adjusted_meta_2") |> filter(pvarname == pvarn) |> select(estimate,estimate_unadjusted,sig_levels) |> collect()
    ...
    output$by_exposure_r2_plot <- renderPlot({
      evarn <- input$evarname
      to_plot <- tbl(pool, "adjusted_meta_2") |> filter(evarname==evarn) |> select(mean_adjusted_base_r2_diff,mean_unadjusted_r2_diff, sig_levels) |> collect()
      to_plot <- to_plot |> rename(r2_adjusted_vs_base=mean_adjusted_base_r2_diff, r2_adjusted_vs_unadjusted=mean_unadjusted_r2_diff)
      plot_adjusted_r2_cdf(to_plot)
    })
    
    output$by_phenotype_context_plot <- renderPlot({
      pvarn <- input$pvarname
      m <- tbl(pool, "adjusted_meta_2") |> filter(pval_BY < 0.05) 
      e_estimate <- m |> filter(pvarname == pvarn) |> select(estimate,sig_levels) |> collect()
      vardesc <- p_variables |> filter(pvarname == pvarn)  |> pull(pvardesc)
      vardesc <- vardesc[1]
      other_estimates <- m |> filter(pvarname != pvarn) |> select(pcategory, psubcategory,estimate, sig_levels) |> collect()
      other_estimates <- other_estimates |> mutate(cat_subcat=ifelse(!is.na(psubcategory), paste(pcategory, psubcategory, sep="-"), pcategory ))
      other_estimates <- other_estimates |> select(-c(pcategory, psubcategory))
    ...
    output$by_exposure_estimate_plot <- renderPlot({
      evarn <- input$evarname
      to_plot <- tbl(pool, "adjusted_meta_2") |> filter(evarname == evarn) |> select(estimate,estimate_unadjusted,sig_levels) |> collect()
      plot_estimates(to_plot)
    })
    ...
    output$by_exposure_group_volcano_plot <- renderPlot({
      ...
      to_plot <- tbl(pool, "adjusted_meta_2") |> filter(ecategory == ecategory_str[1])
      if(!is.na(subcategory_str[1])) {
        to_plot <- to_plot |> filter(esubcategory == subcategory_str[1])
      }
    ...
    output$by_exposure_group_table <- renderReactable({
      ...
      subcategory_str <- category_and_sub |> pull(esubcategory)
      ecategory_str <- category_and_sub |> pull(ecategory)
      to_react <- tbl(pool, "adjusted_meta_2") |> filter(ecategory == ecategory_str[1])
      if(!is.na(subcategory_str[1])) {
        to_react <- to_react |> filter(esubcategory == subcategory_str[1])
      }
      reactable_summary_stats(to_react)
    })
    ...
    output$by_exposure_group_r2_plot <- renderPlot({
      ...
      to_plot <- to_plot |> select(mean_adjusted_base_r2_diff, mean_unadjusted_r2_diff, sig_levels) |> collect()
      to_plot <- to_plot |> rename(r2_adjusted_vs_base=mean_adjusted_base_r2_diff, r2_adjusted_vs_unadjusted=mean_unadjusted_r2_diff)
      plot_adjusted_r2_cdf(to_plot)
    })
    ...
    output$by_exposure_group_estimate_plot <- renderPlot({
      ...
      to_plot <- to_plot |> select(estimate,estimate_unadjusted,sig_levels) |> collect()
      plot_estimates(to_plot)
    })
    
    
    output$survey_pair_plot <- renderPlot({
      evarn <- input$evarname_inter
      pvarn <- input$pvarname_inter
      plot_pair(table_for_pair(evarn, pvarn), c())
    })
    
    output$survey_pair_table <- renderReactable({
      evarn <- input$evarname_inter
      pvarn <- input$pvarname_inter
      to_react <- table_for_pair(evarn, pvarn) |> select(-c(e_variable_description, p_variable_description))
      if(!("term" %in% colnames(to_react))) {
        to_react <- to_react |> select(-expo_name)
      } else {
        to_react <- to_react |> select(-term)
      }
      reactable(to_react |> arrange(Survey), columns = list(estimate=colDef(format = colFormat(digits=3)),
                                                            std.error=colDef(format=colFormat(digits=3)),
                                                            p.value=colDef(format=colFormat(digits=2))),searchable = FALSE, filterable=FALSE, defaultPageSize = 12) 
    })
    output$heterogeneity_table <- renderReactable({
      evarn <- input$evarname_inter
      pvarn <- input$pvarname_inter
      meta_dat <- tbl(pool, "adjusted_meta_2") |> 
        filter(evarname == evarn, pvarname == pvarn) |> select(expo_name, mean_adjusted_base_r2_diff,i.squared, tau.squared, cochran.qe, cochran.qm, p.value.cochran.qm) |> collect()
      
      meta_dat <- meta_dat |> rename(mean_rsq=mean_adjusted_base_r2_diff)
      if(nrow(meta_dat) == 1) {
        meta_dat <- meta_dat |> select(-expo_name)
      }
      reactable(meta_dat, columns = list(mean_rsq=colDef(format = colFormat(digits=3)),
                                         i.squared =colDef(format = colFormat(digits=1)),
                                         tau.squared = colDef(format=colFormat(digits=2)),
                                         cochran.qe = colDef(format=colFormat(digits=2)),
                                         cochran.qm = colDef(format=colFormat(digits=2)),
                                         p.value.cochran.qm=colDef(format=colFormat(digits=2))),searchable = FALSE, filterable=FALSE) 
    })
