...
xxxxx_g <- samp_size_pp_alpha_g |> group_by(yvarname,y_table_name) |> summarize(sd_ss=sd(n), m_ss=mean(n)) |> filter(m_ss >= 500) |> rename(n=m_ss) |> select(-sd_ss) |> ungroup() |> rename(xvarname = yvarname, x_table_name = y_table_name)

yyyyy_g <- select_ubiome |> filter(Begin.Year == 2011) |> select(Variable.Name, Data.File.Name) |> rename(yvarname = Variable.Name, y_table_name=Data.File.Name)

series_g <- cross_join(xxxxx_g, yyyyy_g)

xxxxx_f <- samp_size_pp_alpha_f |> group_by(yvarname,y_table_name) |> summarize(sd_ss=sd(n), m_ss=mean(n)) |> filter(m_ss >= 500) |> rename(n=m_ss) |> select(-sd_ss) |> ungroup() |> rename(xvarname = yvarname, x_table_name = y_table_name)

yyyyy_f <- select_ubiome |> filter(Begin.Year == 2009) |> select(Variable.Name, Data.File.Name) |> rename(yvarname = Variable.Name, y_table_name=Data.File.Name) 
