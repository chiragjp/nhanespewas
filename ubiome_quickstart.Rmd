...
```{r}
rsv_relative <- tbl(con, 'DADA2RB_GENUS_RELATIVE_F') |> collect() |> rbind(tbl(con, 'DADA2RB_GENUS_RELATIVE_G') |> collect())
...
rsv_relative <- compute_clr(rsv_relative)
...
etables <- get_table_names_for_varname(con, varname = "RB_genus780_relative") |> rename(e_name = Data.File.Name)
...
plot(tab_obj$merged_tab$RB_genus780_relative, tab_obj$merged_tab$BMXBMI)
