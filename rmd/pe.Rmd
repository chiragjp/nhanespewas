...
e_bonf_group_summary <- adjusted_meta_2 |> filter(sig_levels == 'Bonf.<0.05') |> unite(e_scategory, c(ecategory, esubcategory)) |> group_by(e_scategory) |> arrange(pvalue_bonferroni) |>  
  summarize(median_bonf_r2=mean(mean_adjusted_base_r2_diff, na.rm=T),  
            median_bonf_abs_estimate=median(abs(estimate), na.rm=T),
            median_bonf_pvalue=median(pvalue_bonferroni, na.rm=T), 
            median_bonf_i.squared=median(i.squared, na.rm=T),
            median_bonf_r2=median(mean_adjusted_base_r2_diff))

## merge
e_group_summary <- e_group_summary |> left_join(e_bonf_group_summary, by='e_scategory')

e_group_summary <- e_group_summary |> filter(n_sig > 1) |> write_csv(file="e_group_summary.csv")


reactable(e_group_summary, defaultPageSize = 25,columns=list(
    e_scategory=colDef(name="Exposure Group"),
    #median_r2 = colDef(name="Median R2", format=colFormat(digits=3)),
    #median_abs_estimate=colDef(name="Median Effect Size", format=colFormat(digits=3)),
    #median_i.squared=colDef(name="Median I2", format=colFormat(digits=1)),
    n_sig=colDef(name="N (p<Bonf)"),
    n_sig_pct=colDef(name="% (p<Bonf)", format=colFormat(digits=1)),
    median_bonf_r2=colDef(name="Median R2 (p<Bonf)", format=colFormat(digits=3)),
    median_bonf_abs_estimate=colDef(name="Median Effect Size (p<Bonf)", format=colFormat(digits=3)),
    median_bonf_pvalue=colDef(name="Median P.value (p<Bonf)", format=colFormat(digits=3)),
    median_bonf_i.squared=colDef(name="Median i2 (p<Bonf)", format=colFormat(digits=1)),
    max_pvarname=colDef(name="Phenotype (lowest p)"),
    max_evarname=colDef(name="Exposure (lowest p)"),
    max_r2 = colDef(name="R2 (lowest p)", format=colFormat(digits=3)),
    max_estimate = colDef("Effect Size (lowest p)", format=colFormat(digits=3)),
    max_p.value = colDef("Lowest p", format=colFormat(digits=3)),
    max_i.squared = colDef("I2 (lowest p)", format=colFormat(digits=1))
  )
)
