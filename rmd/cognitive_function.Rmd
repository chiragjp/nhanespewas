...
cog_decline_row <- summary_stats  |> filter(term == 'expo' | term == 'expo1') |> select(evarname, pvarname, estimate, p.value) |> rename(estimate_overall=estimate, p.value_overall=p.value) 

adjusted_meta_to_hm <- adjusted_meta_to_hm |> rbind(cog_decline_row) 
adjusted_meta_to_hm <- adjusted_meta_to_hm |> mutate(estimate_overall= ifelse(p.value_overall >= 0.1, 0, estimate_overall)) |> select(-p.value_overall)
## 
to_array <- adjusted_meta_to_hm |> pivot_wider(names_from = pvarname, values_from = estimate_overall)

phenome_correlation <- to_array |> select(-evarname) |> correlate(diagonal = 1)

cfdds_architecture <- phenome_correlation |> select(term, CFDDS) |> rename(pvarname=term) |> left_join(adjusted_meta_2 |> select(pvarname, pvardesc) |> unique())
cfdds_architecture <- cfdds_architecture |> mutate(pvardesc= fct_reorder(pvardesc,CFDDS))|> filter(pvarname != 'CFDDS')

p <- ggplot(cfdds_architecture |> filter(abs(CFDDS) > .2, pvarname != "LBXSEL"), aes(pvardesc, CFDDS)) 
p <- p + geom_bar(stat="identity") + ylab("Correlation") + coord_flip() + theme(axis.text.x = element_text(size=9)) + theme_bw()
p
```
