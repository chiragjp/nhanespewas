...
```{r connect to db}
con <- DBI::dbConnect(RSQLite::SQLite(), dbname='../db/pp_summary_stats_03_2024.sqlite')
...
adjusted_meta_raw <- adjusted_meta |> filter(model_number == 1) |> collect() |> left_join(expos_wide_summary, by=c("xvarname", "yvarname")) 

y_variable_domain <- variable_domain |> filter(epcf == 'p') |> collect() |> group_by(Variable.Name) |> summarise(yvardesc=first(Variable.Description),ycategory=first(category),ysubcategory=first(subcategory))
x_variable_domain <- variable_domain |> filter(epcf == 'p') |> collect() |> group_by(Variable.Name) |> summarise(xvardesc=first(Variable.Description),xcategory=first(category),xsubcategory=first(subcategory))
...
cor1 <- cor_matr[category_colors_cor |> arrange(category) |> pull(varname), category_colors_cor |> arrange(lab) |> pull(varname)]
category_colors_cor <- category_colors_cor |> arrange(lab)
