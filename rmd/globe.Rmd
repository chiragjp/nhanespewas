...
exposome_globe <- function(corr_tibble, exposure_tibble, file_out=NULL) {
...
eg_a <- exposome_globe(ee_process$adjusted_meta_raw, to_globe)
#eg_a <- exposome_globe(ee_process$adjusted_meta_raw, to_globe, "PanelA.pdf")

#write.csv(eg_a, file="fig5_globe.csv")
...
eg_b <- exposome_globe(ee_process$adjusted_meta_raw, to_globe)
eg_b <- exposome_globe(ee_process$adjusted_meta_raw, to_globe, "PanelB.pdf")

# Source data for Figure 5
#write.csv(eg_b, file="fig5_globe_panelB.csv")
...
p_cdf <- p
ggsave("PanelC.pdf", p_cdf, width = 7, height = 5)
...
### csv file
to_write_p_cdf <- rbind(ee_process$adjusted_meta_raw |> mutate(p.bonf=p.adjust(p.value), p_sig=p.bonf<.05) |> filter(p.bonf < 0.05) |> select(estimate) |> mutate(color="black"),
exposure_corr_for_sig |> select(estimate) |> mutate(color="blue"),
exposure_corr_for_random |> select(estimate) |> mutate(color="grey"))
#write.csv(to_write_p_cdf,file="fig5_panelC.csv")

```


```{r globe for FEV, fig.height=8, fig.width=8}
evar_examples <- c(
  "SPXNFEV1"
)
...
to_globe <- vector("list", length = length(evar_examples))
for(ii in 1:length(evar_examples)) {
  to_globe[[ii]] <- temp_am |> filter(phenotype == evar_examples[ii]) |> slice_min(n=20, order_by = -rsq_adjusted_base_diff)
}

to_globe <- to_globe |> bind_rows() |> select(exposure) |> group_by(exposure) |> count() |> ungroup()

eg_fev <- exposome_globe(ee_process$adjusted_meta_raw, to_globe)
#write.csv(eg_fev, file="ExtData5_FEVglobe.csv")
```
