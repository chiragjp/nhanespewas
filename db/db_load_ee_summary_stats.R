...
path_to_summary <- '../ee_summary_032324'
load(file.path(path_to_summary, 'ee_summary_032324.Rdata'))

domains <- read_csv('../select/variable_domains_ep_2.csv')

con <- DBI::dbConnect(RSQLite::SQLite(), dbname='./ee_summary_stats_03_2024.sqlite')
...
adjusted_meta <- rbind(
  read_rds(file.path(path_to_summary, 'ee_meta_model_1.rds')) |> select(-meta_model) |> ungroup() |> rename(term_name=term) |> unnest(tidied) |> unnest(glanced),
  read_rds(file.path(path_to_summary, 'ee_meta_model_2.rds')) |> select(-meta_model) |> ungroup() |> rename(term_name=term) |> unnest(tidied) |> unnest(glanced)
)
