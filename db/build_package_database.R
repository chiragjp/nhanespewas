...
library(DBI)
library(tidyverse)
to_con <- DBI::dbConnect(RSQLite::SQLite(), dbname='./nhanes_pewas_a-d.sqlite')
...
table_names_epcf |> filter(series %in% c("A", "B", "C", "D")) |> collect() |> nrow()
...
table_names_to_copy <- table_names_epcf |> filter((series %in% c("A", "B", "C", "D"))) |> collect()
