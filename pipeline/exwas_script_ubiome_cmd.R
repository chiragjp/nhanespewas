...
#script to create "srun_me.sh" to execute the batch jobs for ubiome associations
...
to_do <- read_csv(ss_file) |> filter(!grepl("count$", pvarname)) |> group_by(pvarname) |> count() |> ungroup()
# focus on genus



path_scripts_out <- '../scripts'
cmd_path <- '../pipeline/ubiome_exwas.R'
use_sbatch <- TRUE
